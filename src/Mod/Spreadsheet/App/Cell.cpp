...
void Cell::setContent(const char * value)
{
    ...
    if (value) {
        ...
        if (*value == '=') {
            try {
                newExpr = std::make_unique<App::Expression>(App::ExpressionParser::parse(owner->sheet(), value + 1));
            }
            catch (Base::Exception & e) {
                newExpr = std::make_unique<App::Expression>(new App::StringExpression(owner->sheet(), value));
                setParseException(e.what());
            }
        }
        else if (*value == '\'') {
            newExpr = std::make_unique<App::Expression>(new App::StringExpression(owner->sheet(), value + 1));
        }
        else if (*value != '\0') {
            ...
            if (errno == 0) {
                ...
                if (isEndEmpty) {
                    newExpr = std::make_unique<App::Expression>(new App::NumberExpression(owner->sheet(), Quantity(float_value)));
                }
            }
            ...
            if (!newExpr && isStartingWithNumber) {
                try {
                    auto parsedExpr = std::make_unique<App::Expression>(App::ExpressionParser::parse(owner->sheet(), value));
        ...
        if (!newExpr && *value != '\0') {
            newExpr = std::make_unique<App::StringExpression>(new App::StringExpression(owner->sheet(), value));
        }
